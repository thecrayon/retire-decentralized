...
const useRetirementAccount = () => {
  ...
  const { toastMessage, setToastMessage } = useStateContext();
  ...
  const deposit = async ({ depositToAddress, depositAmount }) => {
    ...
    try {
      const depositTx = await regenRetire.deposit(depositToAddress, {
      ...
      setToastMessage({
        ...
        description: error.message.substring(
          error.message.indexOf("reason") + 0,
          error.message.indexOf("reason") + 26
        ),
        status: "error",
      });

      setError(toastMessage?.description);
    }
    setLoading(false);
  };
